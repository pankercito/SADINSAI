...
function FiltroEstado() {
  ...
  if (filter !== '') {
    ...
    var table = document.querySelector(".sede-table"); // Obtener todas las filas de la tabla

    var rows = table.getElementsByTagName("tr"); // Iterar sobre todas las filas de la tabla

    for (var i = 1; i < rows.length; i++) {
      var row = rows[i];
      var cells = row.getElementsByTagName("td");
      var matchFound = false; // Iterar sobre la segunda y tercera celda de la fila

      for (var j = 2; j <= 3; j++) {
        var cellText = cells[j].textContent.toLowerCase(); // Comprobar si el texto de la celda contiene la palabra buscada
...
function FiltroAdmin() {
  ...
  if (filter !== '') {
    // Obtener la tabla por su clase
    var table = document.querySelector(".admin-table"); // Obtener todas las filas de la tabla
    ...
    for (var i = 1; i < rows.length; i++) {
      var row = rows[i];
      var cells = row.getElementsByTagName("td");
      var matchFound = false; // Iterar sobre celdas de la fila
...
function FiltroUser() {
  ...
  if (filter !== '') {
    // Obtener la tabla por su clase
    var table = document.querySelector(".user-table"); // Obtener todas las filas de la tabla
