...
function enableButton() {
  var inputs = [inputName, inputLastName, inputCi, inputDireccion, inputPhone, inputEmail];
  var allInputsLlenos = inputs.every(function (input) {
    return input.value.trim() !== '';
  });
  var allSelectsActivos = estadoSelect.selectedIndex !== 0 && sedeSelect.selectedIndex !== 0 && ciudadSelect.selectedIndex !== 0; //La funcion ".toggle" agrega o quita una clase CSS dependiendo de si la clase ya existe o no
...
[inputName, inputLastName, inputCi, inputDireccion, inputPhone, inputEmail, estadoSelect, ciudadSelect, sedeSelect].forEach(function (element) {
...
function verificarCI() {
  ...
  xhttp.onreadystatechange = function () {
    if (this.readyState == 4 && this.status == 200) {
      var respuesta = this.responseText; //no se encuentra en ninguna tabla
      ...
      if (respuesta == "!registra primero¡") {
        document.getElementById("mensajeCi").innerHTML = "";
          ...
          document.getElementById("mensajeCi").innerHTML = "<p id=\"pmsj\" class='pmsj'>\xA1La cedula ingresada ya est\xE1 registrada!</p>";
